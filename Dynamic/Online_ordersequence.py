...
def opt_tsp_solver(current_coordinate, packing_coordinate, opt_nodes, new_nodes, node_point_y_x):
    ...
    #best_route = opt_check(best_route, first_point, end_point, node_point_y_x)
...
def opt_check(input_route, first_point, end_point, node_point_y_x):
    action_flag = True
    ...
    while action_flag:
        ...
        for ind1, m_edge in enumerate(route):
            opt_score = math.inf

            for ind2, c_edge in enumerate(route[next_point:]):
                sub_route = copy.deepcopy(route)
                change_edge1 = [m_edge[0], c_edge[0]]
                change_edge2 = [m_edge[1], c_edge[1]]
                sub_route[ind1] = change_edge1
                sub_route[ind2 + ind1] = change_edge2
                ...
                if opt_score > score:
                    opt_score = score
                    opt_route = resolve_route(sub_route)
