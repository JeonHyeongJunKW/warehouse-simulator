...
def static_aco_tsp_solver(current_position,packing_position,tsp_node, node_point_y_x):
    ...
    DEBUG_log_tag("피킹지점들", np.array(node_point_y_x)[:,1][tsp_node].tolist(),"VERY_DETAIL")
    DEBUG_log_tag("돌아올 지점", [packing_position[1]],"VERY_DETAIL")
    heuristic_vector_x = np.array([current_position[1]]+np.array(node_point_y_x)[:,1][tsp_node].tolist()+[packing_position[1]])
    heuristic_vector_y = np.array([current_position[0]]+np.array(node_point_y_x)[:,0][tsp_node].tolist()+[packing_position[0]])
    ...
    DEBUG_log_tag("heuristic_matrix example",np.round_(heuristic_map,2).tolist(),"VERY_DETAIL")
    ...
    while True:
        ...
        for ant_ind in range(ant_size):
            ...
            is_not_go.remove(packing_index)  # 0번도시를 제거
        ...
        if cur_it > max_it:
            break
    DEBUG_log_tag("초기경로", tsp_node, "VERY_DETAIL")
    DEBUG_log_tag("나중경로", best_path, "VERY_DETAIL")
    DEBUG_log_tag("초기경로길이", init_length, "VERY_DETAIL")
    DEBUG_log_tag("나중경로길이", best_ant_length, "VERY_DETAIL")
